(function () {
  ...
  function recordFromNode(node) {
    ...
    var recordId = String(data.shdRecordId || '').toLowerCase();
    if (recordId) {
      var exact = state.records.find(function (item) {
        return text(item.id || item.code || item.key || item.number).toLowerCase() === recordId;
      });
      if (exact) return exact;
    }
    var recordTitle = String(data.shdRecordTitle || '').toLowerCase();
    if (recordTitle) {
      var titled = state.records.find(function (item) {
        return text(item.title || item.name || item.display_name || item.number).toLowerCase() === recordTitle;
      });
      if (titled) return titled;
    }
    var ariaLabel = node && node.getAttribute ? String(node.getAttribute('aria-label') || '').toLowerCase() : '';
    return state.records.find(function (item) {
      var title = text(item.title || item.name || item.display_name || item.number).toLowerCase();
      return title && title === ariaLabel;
    }) || null;
  }
  ...
  function update(value) {
    ...
    if (records.length || Array.isArray(payload.data) || Array.isArray(payload.items) || Array.isArray(payload.projects)) {
      state.records = records;
      if (state.selected && !state.records.some(function (item) {
        var selectedId = text(state.selected.id || state.selected.code || state.selected.key || state.selected.number).toLowerCase();
        var itemId = text(item.id || item.code || item.key || item.number).toLowerCase();
  ...
  window.addEventListener('shd-widget-data', function (event) {
    update(event && event.detail);
  });

  window.__SHD_WIDGET_ACTIONS_UPDATE__ = update;
  ...
  ensurePanel();
  var initial = typeof window !== 'undefined' && (window.__SHD_WIDGET_STATE__ || (window.openai && (window.openai.toolOutput || window.openai.toolInput)));
