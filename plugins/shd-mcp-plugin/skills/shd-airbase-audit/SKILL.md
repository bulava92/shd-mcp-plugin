...
2. If no table is named, call `shd_airbase_tables` first and select only the
   relevant tables. Do not load every table by default.
3. Call `shd_airbase_schema` for each selected table before reading its
   records. Pass the table explicitly when a single-table audit is requested.
4. Call `shd_airbase_records` with bounded reads and the selected table.
